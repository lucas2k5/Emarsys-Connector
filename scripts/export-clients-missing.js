...
/**
 * Extração complementar — apenas Jul/2024 → Dez/2026 em fatias trimestrais.
 * Mescla com o CSV existente e deduplica por customer_id.
 *
 * Uso: node scripts/export-clients-missing.js
 */
...
// Fatias trimestrais do período faltante
const DATE_SLICES = [
...
const DELAY_BETWEEN_SLICES_MS = 3 * 60 * 1000;
...
(async () => {
  console.log('\n📦 Extração complementar — Jul/2024 → Dez/2026 (fatias trimestrais)\n');
  ...
  if (!existingCsv) {
    console.error('❌ Nenhum CSV existente encontrado em tmp/. Rode export-clients-full.js primeiro.');
    process.exit(1);
  }
  ...
  for (let i = 0; i < DATE_SLICES.length; i++) {
    ...
    if (i < DATE_SLICES.length - 1) {
      process.stdout.write(`\n   ⏳ Aguardando 3 min para sessão expirar na VTEX...`);
      await sleep(DELAY_BETWEEN_SLICES_MS);
    }
  }

  // Deduplicar: só adicionar IDs que não estão no CSV existente
