// Vercel Cron Job para sincronização combinada de pedidos e produtos VTEX
...
module.exports = async function handler(req, res) {
  ...
  console.log('🕐 [Vercel Cron] Iniciando sincronização combinada VTEX...');
  
  try {
    const startTime = Date.now();
    
    // Chamar diretamente o serviço de sincronização completa
    const syncService = require('../../utils/syncService');
    const result = await syncService.executeFullSync();
    
    const duration = Date.now() - startTime;
    console.log(`✅ Sincronização combinada concluída em ${duration}ms`);
